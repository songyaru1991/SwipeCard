		...
		// for ( var i = 0; i < 1; i++) {
			...
			if (cal == "1") {  //正常班
				...
				//console.log("sdate1[0]:"+ sdate1[0]);{
					sdate1[i].setHours(17, 30, 0);  //setHours(hour,min,sec,millisec) 方法用于设置指定的时间的小时字段。
				}else{
				...
				if(shift=="D"){
					for(var j = 0;j<t_s.length;j++){  //t_s[]休息间隔
						...
						if(j<t_s.length-1){
							...
							tempHour1 = t_s[j].split(":");          //[17,30]
							tempHour2 = edate1[i];                  //下班时间
						}
						...
						// tempTime1 = sdate1[i];
						var	tempTime1 = new Date(sdate1[i]);   //tempTime1设置为当前上班刷卡的日期
						tempTime1 = tempTime1.setHours(tempHour1[0],tempHour1[1],0);   //tempHour1设置为当前上班时段的起始时间，如7:40
						// console.log("T1 :" + tempTime1);
						var	tempTime2 = new Date(edate1[i]);
						if(j<t_s.length-1){
							tempTime2 = tempTime2.setHours(tempHour2[0],tempHour2[1],0); //tempHour2设置为当前上班时段的终止时间，如09:30
						}
